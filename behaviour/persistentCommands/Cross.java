...
public class Cross extends Leaf {

    @Override
    public boolean isValid() {
        return API.mode == API.modes.CROSS;
    }

    @Override
    public int onLoop() {		
		List<Player> followID = Players.all(p-> p.getName().toLowerCase().contains(API.callerName.toLowerCase()) &&
				!p.equals(Players.localPlayer()));
		if(followID.isEmpty())
		{
			MethodProvider.log("CROSS target: " + API.callerName + " not found! Waiting...");
		...
		{
    		MethodProvider.log("CROSS target left! Exiting CROSS...");
			API.exitstop();
    		return Sleep.Calculate(333,666);
		}
    	if(API.newShape == true)
		{
			
    		int randint = API.rand2.nextInt(13);
    		switch(randint)
    		{
    		...
    		case(7):{API.yn = -3; break;}
    		case(8):{API.yn = -2; break;}
    		case(9):{API.yn = -1; break;}
    		case(10):{API.yn = 1; break;}
    		case(11):{API.yn = 2; break;}
    		case(12):{API.yn = 3; break;}
    		}
    		API.newShape = false;
		}
